...
const SpecimenView = ({ data, dataType, darkMode = false, onClose, onSave }) => {
  ...
  const fetchReferenceData = async (speciesId) => {
    ...
    try {
      ...
      console.log('Reference loaded:', response.data.common_name || response.data.species_name);
    } catch (error) {
  ...
  const refreshSpecimenData = async () => {
    const specimenId = currentData.compressive_id || 
    ...
    if (specimenId) {
      try {
        ...
        console.log('SpecimenView: Fresh data received, species_id:', response.data.species_id);
  ...
  const calculateExperimentalStress = () => {
    ...
    const maxForce = parseFloat(currentData.max_force) || parseFloat(currentData?.['Maximum Force']) || 0;
  ...
  return (
    <div className={`w-full h-full ${darkMode ? 'bg-gray-800' : 'bg-white'}`}>
      {/* Header */}
      <div className={`flex items-center justify-between px-4 py-2 border-b ${
        ...
        <h2 className={`text-lg font-semibold ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
          View | {getTestTypeLabel()} | {currentData?.specimen_name || currentData?.['Specimen Name'] || 'Unknown'}
        </h2>
      ...
      <div className="grid grid-cols-2 h-[calc(100%-48px)]">
        ...
        <div className={`flex flex-col items-center justify-center border-r ${
          ...
          <div className={`text-7xl mb-4 ${darkMode ? 'text-gray-600' : 'text-gray-300'}`}>📷</div>
          <div className={`mt-4 text-2xl ${darkMode ? 'text-gray-500' : 'text-gray-400'}`}>⌜ ⌟</div>
        </div>

        {/* RIGHT COLUMN - Information (scrollable) */}
        <div className="overflow-y-auto">
          {/* 1. Accuracy Percentage - FIRST */}
          <div className={`px-4 py-3 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
          ...
          <div className={`grid grid-cols-2 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
            <div className={`px-4 py-3 border-r ${darkMode ? 'border-gray-700' : 'border-black'}`}>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Specimen Name :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.specimen_name || currentData?.['Specimen Name'] || 'Unknown'}
              </div>
            </div>
            <div className="px-4 py-3">
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Reference Species :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {loading ? 'Loading...' : (referenceData?.common_name || referenceData?.species_name || referenceData?.botanical_name || 'No reference selected')}
              </div>
            </div>
          </div>
          ...
          <div className={`grid grid-cols-2 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
            <div className={`px-4 py-3 border-r ${darkMode ? 'border-gray-700' : 'border-black'}`}>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Calculated Stress (σ = P/A) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-blue-400' : 'text-blue-600'}`}>
                {experimentalStress.toFixed(2)} MPa
              </div>
            </div>
            <div className="px-4 py-3">
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Reference Stress :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
          ...
          <div className={`grid grid-cols-2 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
            <div className={`px-4 py-3 border-r ${darkMode ? 'border-gray-700' : 'border-black'}`}>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Maximum Force (P) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.max_force || currentData?.['Maximum Force'] || ''} N
              </div>
            </div>
            <div className="px-4 py-3">
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Moisture Content (%) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.moisture_content || currentData?.['Moisture Content'] || '-'} %
              </div>
            </div>
          </div>
          ...
          <div className={`grid grid-cols-2 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
            <div className={`px-4 py-3 border-r ${darkMode ? 'border-gray-700' : 'border-black'}`}>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Base (b) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.base || ''} mm
              </div>
            </div>
            <div className="px-4 py-3">
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Height (h) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
          ...
          <div className={`grid grid-cols-2 border-b ${darkMode ? 'border-gray-700' : 'border-black'}`}>
            <div className={`px-4 py-3 border-r ${darkMode ? 'border-gray-700' : 'border-black'}`}>
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Length (L) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.length || ''} mm
              </div>
            </div>
            <div className="px-4 py-3">
              <div className={`text-xs ${darkMode ? 'text-gray-400' : 'text-gray-600'}`}>Area (A = b × h) :</div>
              <div className={`mt-1 text-base font-medium ${darkMode ? 'text-gray-100' : 'text-gray-900'}`}>
                {currentData?.area || ''} mm²
              </div>
            </div>
