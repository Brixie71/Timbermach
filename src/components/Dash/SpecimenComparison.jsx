...
const SpecimenComparison = ({
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      const filtered = allReferenceData.filter(
        (item) =>
          item.species_name
            ?.toLowerCase()
  ...
  const fetchAllReferenceData = async () => {
    ...
    try {
      const response = await axios.get(
        `http://127.0.0.1:8000/api/reference-values`,
      );
  ...
  const calculateExperimentalStress = () => {
    if (!data) return 0;
    const maxForce =
      parseFloat(data.max_force) || parseFloat(data?.["Maximum Force"]) || 0;
  ...
  const handleSaveComparison = async () => {
    ...
    try {
      const specimenId =
        data.compressive_id || data.shear_id || data.flexure_id;
      ...
      const updatePayload = {
        ...
        moisture_content:
          data.moisture_content || data["Moisture Content"] || null,
  ...
  const percentageDifference =
    referenceStress > 0
      ? (
          ((experimentalStress - referenceStress) / referenceStress) *
          100
        ).toFixed(2)
      : 0;
  const comparisonRatio =
    referenceStress > 0 ? (experimentalStress / referenceStress).toFixed(3) : 0;

  return (
    <div
      className={`w-full h-full flex flex-col ${darkMode ? "bg-gray-800" : "bg-gray-50"}`}
    >
      {/* Fixed Header */}
      <div
      ...
      >
        ...
        >
          Comparison |{" "}
          {data?.specimen_name || data?.["Specimen Name"] || "Unknown"}
        </h2>
        <button
          onClick={onClose}
        ...
        <div>
          ...
          <div className="relative border-b ${darkMode ? 'border-gray-700' : 'border-black'}">
            ...
            {showDropdown && searchQuery.trim() && (
              <div
                className={`absolute top-full left-0 right-0 max-h-60 overflow-y-auto border shadow-lg z-10 ${
                  darkMode
                  ...
                  filteredData.map((species, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {species.botanical_name &&
                          species.botanical_name !== species.common_name && (
                            <div
          ...
          {selectedSpecies ? (
            <>
              ...
              <div
                className={`px-4 py-3 border-b ${darkMode ? "bg-gray-900 border-gray-700" : "bg-gray-50 border-black"}`}
              >
                <div
            ...
            >
              ...
              <p className="text-sm">
                Search and select a wood species to compare
              </p>
            </div>
          )}
        </div>
