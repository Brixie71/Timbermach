...
class ContourDetectionPipeline:
    ...
    def __init__(self, camera_index=0, window_name="Contour Detection Pipeline"):
        ...
        self.default_params = {
            ...
            "mm_per_pixel": 0.1288,  # overridden by API presets per test type
    ...
    def get_trackbar_values(self):
        ...
        roi_shape = self.default_params.get("roi_shape", "square")
    ...
    def calculate_roi(self, frame_shape, roi_size_percent, roi_shape="square"):
        h, w = frame_shape[:2]
        size_pct = roi_size_percent / 100.0
        roi_shape = (roi_shape or "square").lower()

        if roi_shape == "rectangle":
            roi_w = int(w * size_pct)
            roi_h = int(h * size_pct)
        else:
            square = int(h * size_pct)
            square = min(square, w)
            roi_w = square
            roi_h = square
    ...
    def create_info_overlay(self, img, contour_count, params, roi, actual_fps, proc_ms, fps_mode):
        ...
        lines = [
            ...
            (f"ROI: {roi[2]}x{roi[3]} ({params['roi_size']}%, {params.get('roi_shape','square')})", (0, 255, 0)),
