...
class SevenSegmentOCR:
    ...
    def set_calibration(self, display_box: Dict, segment_boxes: List[List[Dict]], 
                       has_decimal_point: bool = False, decimal_position: int = 1):
    ...
    def detect_segment_simple_threshold(self, gray_roi: np.ndarray, 
                                       is_inverted: bool = False) -> Tuple[bool, Dict]:
        """
    ...
    def detect_segment_state_smart_adaptive(self, gray_roi: np.ndarray, 
                                            is_inverted: bool = False) -> Tuple[bool, Dict]:
        """
        ...
        if is_inverted:
            ...
            is_on = (
                contrast > 30 and
                mean_dark < median_brightness - 15 and
                mean_brightness < 150
            )
        else:
            # Light digits on dark background
            bright_pixels = gray_roi[gray_roi > p75]
            mean_bright = float(np.mean(bright_pixels)) if len(bright_pixels) > 0 else max_val
            
            is_on = (
                contrast > 30 and
                mean_bright > median_brightness + 15 and
                mean_brightness > 80
            )
    ...
    def check_segment_active(self, image: np.ndarray, segment_box: Dict, 
                            display_box: Dict, is_inverted: bool = False,
                            method: str = None) -> Tuple[bool, Dict]:
    ...
    def recognize_digit(self, image: np.ndarray, segment_boxes: List[Dict], 
                       display_box: Dict, is_inverted: bool = False,
                       method: str = None) -> Tuple[str, str, List[bool], List[Dict]]:
        """Recognize a single digit from segment boxes"""
        if len(segment_boxes) != 7:
            raise ValueError("Expected 7 segment boxes")
    ...
    def recognize_display(self, image: np.ndarray, debug: bool = False) -> Dict:
        ...
        for digit_idx, segment_boxes in enumerate(self.calibration['segment_boxes']):
            ...
            digit_result = {
                ...
                'segments': {
                    ...
                    'D': states[3], 'E': states[4], 'F': states[5], 'G': states[6]
                }
            }
        ...
        response = {
            ...
            'decimal_position': self.decimal_position if self.has_decimal_point else None,
            'digits': results,
    ...
    def visualize_segments(self, image: np.ndarray, save_path: str = None) -> np.ndarray:
        """Draw segment boxes on the image for visualization"""
        return self.visualize_segments_with_binary(image)


def create_default_segment_boxes(display_box: Dict, num_digits: int = 3) -> List[List[Dict]]:
