...
class ContourDetectionPipeline:
    ...
    def __init__(self, camera_index=0, window_name="Contour Detection Pipeline"):
        ...
        self.default_params = {
            ...
            'dilation': 1,
            'erosion': 1,
            'roi_size': 60,  # Percentage of frame height (100% = full height square)
            'brightness': 99,  # -100 to 100
            'contrast': 101,   # 0 to 200 (100 = no change)
            'mm_per_pixel': 0.1  # Default calibration (adjust based on camera setup)
        }
        
        # ROI will be calculated based on frame dimensions
        self.roi_rect = None
        self.show_crosshair = True  # Toggle for center alignment crosshair
        self.use_bilateral_filter = False  # Toggle for bilateral filter (better noise reduction)
    ...
    def process_frame(self, img, params):
        ...
        contours, hierarchy = cv2.findContours(img_processed, 
                                               cv2.RETR_EXTERNAL, 
